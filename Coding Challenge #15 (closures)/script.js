'use strict';

/*
Take the IIFE below and at the end of the function,
attach an event listener that changes the color of 
the selected h1 element ('header') to blue, each time
the BODY element is clicked. Do NOT select the h1 
element again!

And now explain to YOURSELF (or someone around you)
WHY this worked! Take all the time you need. Think 
about WHEN exactly the callback function is executed,
and what that means for the variables involved in this
example.

GOOD LUCK! :) 

*/

(function () {
  const header = document.querySelector('h1');
  header.style.color = 'red';

  //so how did this callback function get access to the 'header' variable
  //through closures!
  document.querySelector('body').addEventListener('click', function () {
    //this function is still able to access the header variable
    //because it has access to the any variables related to it's birthplace
    //'header' is in the backpack of this method.
    header.style.color = 'blue';
  });
})();
